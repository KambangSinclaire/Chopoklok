...
const getCartCurrentUser = async () => {

  // this fonction return current user cart in firebase
  // const cart = await firebase.find('carts', {email: 'user email'})
};
...
const addToCart = async (id) => {
  ...
  if (product) {
    ...
    const user_cart =  await getCartCurrentUser();
    // If user have the cart and check if is_pay === false, just update content cart
    //  Esle create a new cart

    const firebase_cart_format = {
      email: "user email here",
      products: [product],
      is_pay: false,
    };
    // Get login user
    // Format data to send firebase
    // Save in firebase const save = await firebase.save(firebase_cart_format);
    console.log(product);
  }
}; */
